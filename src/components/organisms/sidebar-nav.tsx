...
export function SidebarNav({ role, orgName, userName, avatarUrl }: SidebarNavProps) {
  const pathname = usePathname();
  const items = PRIMARY_NAV.filter((i: NavItem) => !i.roles || (role && i.roles.includes(role)));
  const profileActive = isActive(pathname, "/perfil");
  ...
  return (
    <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col border-r border-outline-variant bg-surface-container-lowest lg:flex">
      ...
      <div className="border-t border-outline-variant p-3">
        ...
        >
          <span className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-full bg-surface-container-high text-on-surface-variant">
            {avatarUrl ? (
              <Image
                src={avatarUrl}
                alt=""
